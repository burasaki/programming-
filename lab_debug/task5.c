...
void xor_encrypt(char *text, int text_len, char *key, char *output) {
...
void caesar_cipher(char *text, int shift, char *output) {
    ...
    int i = 0;
    for (; i < len; i++) {
        if (text[i] >= 'a' && text[i] <= 'z') { // было > и <
            output[i] = ((text[i] - 'a' + shift + 26) % 26) + 'a'; // не было + 26
...
void vigenere_encrypt(char *text, char *keyword, char *output) {
    ...
    for (int i = 0; i < text_len; i++) {
        if (isalpha(text[i])) {
            ...
            output[i] = ((text[i] - 'A' + shift + 26) % 26 + 'A'); // text[i] + shift;
            key_index++;
        } else {
            output[i] = text[i];
        }
...
void simple_rc4(char *key, int key_len, char *data, int data_len) {
    ...
    for (i = 0; i < 256; i++) { // тут было 256 + 32
...
unsigned int simple_hash(char *str) {
    ...
    for (int i = 0; i < strlen(str); i++) { //был sizeof
        hash += str[i] * (i + 1) + 50;
    }
...
void test_vigenere_cipher() {
    ...
    printf("Keyword: %s\n", keyword);
    printf("Encrypted: %s\n", output);
    vigenere_decrypt(output, keyword, output);
    printf("Decrypted: %s\n", output);
}
...
int main() {
    ...
    srand(time(NULL)); // было в функции
    test_key_generation();
